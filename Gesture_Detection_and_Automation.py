...
while(1):
    ...
    try:      
        ...
        hsv = cv2.cvtColor(roi, cv2.COLOR_BGR2HSV) # Conversão RGB => HSV     
    ...
    # Dilatação na mão para preencher manchas escuras
   ...
   # Encontrar o contorno máximo da mão
    ...
    # Definir área of hull 
        areahull = cv2.contourArea(hull)
        areacnt = cv2.contourArea(cnt)
      
    # Definir área do objeto e área da mão
        arearatio = ((areahull-areacnt)/areacnt)*100
    
    # Encontrar os defeitos no objeto convexo em relação à mão
    ...
    # Defininir a região de interesse
        for i in range(defects.shape[0]):
            ...
            # Ignorar ângulos > 90 e ignorar pontos muito próximos ao objeto convexo
        ...
        if l==1:
            if areacnt < 2000: # Sem objetos na região
                ...
                if arearatio < 12 and not executado:
                    ...
                    #os.system("start VS Code vscode.exe")
            ...
            #os.system("start spotify.exe --window-size=600,400")
            #break
            ...
            #os.system("start hamachi.exe)

        else :
            cv2.putText(frame,'reposition',(10,50), font, 2, (0,0,255), 3, cv2.LINE_AA)
            #os.system("start paint.exe)
